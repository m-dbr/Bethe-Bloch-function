...
Double_t BetheBlochVsBG(Double_t *x, Double_t *par);
...
void FitSlices(Int_t slic=1, Int_t runid=1234, Int_t apver=4, Int_t nSlices=15, Int_t saveCalibParams=0)
{
  ...
  Double_t X0=1.44;  // from Gabor thesis
  Double_t X1=4; 
  Double_t miu=3.77; 
  Double_t s=1.53;
...
//   PION
  ...
  Double_t mass_mu = 0.10565837;

  gBethe_Bloch_mu = new TF1("Bethe Bloch mu", BetheBloch, -0.5, 3, 5);
  gBethe_Bloch_mu->SetParameters(par_pi);
  gBethe_Bloch_mu->SetParNames("X0", "X1", "miu", "s", "mass");
  gBethe_Bloch_mu->FixParameter(4, mass_mu);
...
//
  ...
  Double_t mass_elect = 0.511e-3;
//
  Double_t par_elect[] = {X0, X1, miu, s, mass_elect};

  gBethe_Bloch_el = new TF1("Bethe Bloch pi", BetheBloch, -0.5, 3, 5);
  gBethe_Bloch_el->SetParameters(par_elect);
  gBethe_Bloch_el->SetParNames("X0","X1","miu","s", "mass_elect");
  gBethe_Bloch_el->FixParameter(4, mass_elect);
...
// Take PIONS from root file
  ...
  //TH2F* hr_pi = (TH2F*)InputFile->Get(Form("%s/hPiPlus_log",dir2.Data()));
...
// Slicing function for pions/protons
  ...
  Double_t par_pion[] = {par1,par2,par3,par4}; 

  Double_t par_prot[] = {0,0,0,0}; 
  if ( slic == 0 ) {
      string particle2 = "proton";
      gPmin=1.0;
      gPmax=1.4;
      DyVsY2(nSlices,hr, particle2, par1, par2, par3, par4);
      par_prot[0] = par1; par_prot[1] = par2; par_prot[2] = par3; par_prot[3] = par4; 
  }

  // original Gabor thesis parameters
  //    Double_t X0=1.44;
  //    Double_t X1=4;
  //    Double_t miu=3.77;
  //    Double_t s=1.53;

      cout << "!!!!!!!!!!!!!! fit for pions:" << endl;
      cout <<"X0  = "<< par_pion[0] << endl;
      cout <<"X1  = "<< par_pion[1] << endl;
      cout <<"miu = "<< par_pion[2] << endl;
      cout <<"s   = "<< par_pion[3] << endl;
      cout<<endl;
      cout << "!!!!!!!!!!!!!! fit for protons:" << endl;
      cout <<"X0  = "<< par_prot[0] << endl;
      cout <<"X1  = "<< par_prot[1] << endl;
      cout <<"miu = "<< par_prot[2] << endl;
      cout <<"s   = "<< par_prot[3] << endl;
      cout << "!!!!!!!!!!!!!!" << endl;
  ...
  mean_pi     -> SetMarkerSize(1.3);
  mean_pi     -> SetMarkerStyle(20);

  TF1* Bethe_Bloch_pi    = (TF1*)list_pi->FindObject(Form("Bethe_Bloch_pion"));
  
  
  cout<<"parameters from Bethe_Bloch_pi:"<<endl;
  ...
  if (slic == 0 ) {
    ...
    Bethe_Bloch_p    = (TF1*)list_p->FindObject(Form("Bethe_Bloch_proton"));

    cout<<"parameters from Bethe_Bloch_p:"<<endl;
  ...
  mean_p     -> SetMarkerSize(1.3);
  mean_p     -> SetMarkerStyle(20);
  ...
  hrplot->Draw(""); // pions
  hr->Draw("same"); // protons
  mean_pi->Draw("same p");
  mean_p->Draw("same p");

  //gBethe_Bloch_k->Draw("same");
  //gBethe_Bloch_mu->Draw("same");
  //gBethe_Bloch_el->Draw("same");

  cout<<"electron dEdx = "<<gBethe_Bloch_el->Eval(1.)<<endl;

    // Bethe Bloch for pi with parameters from proton fit
    Double_t par_p_from_pion[] = {par_pion[0], par_pion[1], par_pion[2], par_pion[3], mass_prot};
...
//---------------------------------------------------------------------
    ...
    cout<<"---------------------------> Entering Minuit block <--------------------------------------"<<endl;
        const Int_t npars = 4;
...
// Print results
	...
	/// from fit to MeanVsBG for combined pions and protons
	currentPar[0] = -2.64986e-01; currentPar[1] = 7.27216; currentPar[2] = 1.47865; currentPar[3] = 1.47865;
	...
	// test sigma dep.
	////////////////////////////////////////////////////////////////////////////
	cout<<"---------------------------> Entering SUMMARY block <--------------------------------------"<<endl;
    
	TCanvas* c2 = new TCanvas ("Sigma","Sigma",150,150,650,560);

	c2->Divide(1,1);
	c2->cd(1);

	TList* list_prot = hr->GetListOfFunctions();
	TList* list_pi   = hr_pi->GetListOfFunctions();
	...
	TGraphErrors* SigmaVsBG_prot   = (TGraphErrors*)list_prot->FindObject(Form("SigmaVsLogBG"));
	TGraphErrors* SigmaVsBG_pi     = (TGraphErrors*)list_pi->FindObject(Form("SigmaVsLogBG"));

	SigmaVsBG_prot->SetLineColor(1);
	mg_sig->Add(SigmaVsBG_prot);
	mg_sig->Add(SigmaVsBG_pi);

	SigmaVsBG_prot->SetMarkerSize(0.9);
	SigmaVsBG_pi->SetMarkerSize(0.9);
	SigmaVsBG_pi->SetMarkerColor(2);

	TH2F* frame = new TH2F("frame","frame",1000,0.9,2.05,1000,0,0.09);
	frame->Draw();
	mg_sig -> Draw("p same");
	...
	//////////////////////////////////////////////////////////////////////
	cout<<"tu1"<<endl;
	TCanvas* c3 = new TCanvas ("MeanVsBG","MeanVsBG",200,200,650,560);
	cout<<"tu2"<<endl;
	...
	TGraphErrors* MeanVsBG_prot   = (TGraphErrors*)list_prot->FindObject(Form("MeanVsLogBG"));
	TGraphErrors* MeanVsBG_pi     = (TGraphErrors*)list_pi->FindObject(Form("MeanVsLogBG"));

	MeanVsBG_prot->SetLineColor(1);
	mg_mean->Add(MeanVsBG_prot);
	mg_mean->Add(MeanVsBG_pi);

	MeanVsBG_prot->SetMarkerSize(0.9);
	MeanVsBG_pi->SetMarkerSize(0.9);
	MeanVsBG_pi->SetMarkerColor(2);

	TH2F* frame2 = new TH2F("frame2","frame2",1000,0.9,2.05,1000,0,0.09);
	frame2->Draw();
	mg_mean -> Draw("p same");


	//////////// make BetheBloch vesus beta*gamma 
	//X0  = -4.641; 
	//X1  =  4.19;
	//miu =  3.246;
	//s   =  2.145; 
	...
	Double_t par2_p[] = {X0, X1, miu, s, mass_prot};
	
	TF1* BetheVsBG_p = new TF1("BetheVsBG_p", BetheBlochVsBG, -0.5, 3, 5); 
	BetheVsBG_p->SetParameters(par2_p);
	BetheVsBG_p->SetParNames("X0", "X1", "miu", "s", "mass");
	BetheVsBG_p->FixParameter(4, mass_prot);

	BetheVsBG_p->SetLineColor(4);
	BetheVsBG_p->Draw("same");
	...
	Double_t par2_pi[] = {X0, X1, miu, s, mass_pi};
	
	TF1* BetheVsBG_pi = new TF1("BetheVsBG_pi", BetheBlochVsBG, -0.5, 3, 5); 
	BetheVsBG_pi->SetParameters(par2_pi);
	BetheVsBG_pi->SetParNames("X0", "X1", "miu", "s", "mass");
	BetheVsBG_pi->FixParameter(4, mass_pi);

	BetheVsBG_pi->SetLineColor(2);
	BetheVsBG_pi->Draw("same");

	BetheVsBG_pip = new TF1("BetheVsBG_pip", BetheBlochVsBG, -0.5, 3, 5); 
	BetheVsBG_pip->SetParameters(par2_pi);
	BetheVsBG_pip->SetParNames("X0", "X1", "miu", "s", "mass");
	BetheVsBG_pip->FixParameter(4, mass_prot);

	BetheVsBG_pip->SetLineColor(3);
	BetheVsBG_pip->SetLineStyle(2);
	BetheVsBG_pip->Draw("same");

	Double_t x_arr[200];
	Double_t y_arr[200];
	Double_t xerr_arr[200];
	Double_t yerr_arr[200];
	// make one prot+pion graph (meanVsBG) and fit it.
	Int_t Np = 0;
	for(Int_t i=0;i<MeanVsBG_prot->GetN();i++){
	  Double_t x;
	  Double_t y;
	  MeanVsBG_prot->GetPoint(i,x,y);
	  cout<<"i="<<i<<" "<<x<<" "<<y<<endl;
	  x_arr[i] = x;
	  y_arr[i] = y;
	  xerr_arr[i] =  MeanVsBG_prot->GetErrorX(i);
	  yerr_arr[i] =  MeanVsBG_prot->GetErrorY(i);
	  Np++;
	}
	cout<<"Np="<<Np<<" "<<MeanVsBG_prot->GetN()<<endl;
	Int_t Ntot = Np;
	Int_t ii=0;
	for(Int_t i=0;i<MeanVsBG_pi->GetN();i++){
	  if(i<3)continue;
	  Double_t x;
	  Double_t y;
	  MeanVsBG_pi->GetPoint(i,x,y);
	  x_arr[ii+Np] = x;
	  y_arr[ii+Np] = y;
	  xerr_arr[ii+Np] =  MeanVsBG_pi->GetErrorX(i);
	  yerr_arr[ii+Np] =  MeanVsBG_pi->GetErrorY(i);
	  Ntot++;
	  ii++;
	}
	
      TGraphErrors *MeanVsBG_pip = new TGraphErrors(Ntot, x_arr, y_arr, xerr_arr, yerr_arr);
      MeanVsBG_pip->SetName("MeanVsBG_pip");
      MeanVsBG_pip->SetMarkerStyle(24);	
      MeanVsBG_pip->SetMarkerColor(42);
      MeanVsBG_pip->SetMarkerSize(1.9);
      MeanVsBG_pip->Draw("same p");

      TCanvas* c4 = new TCanvas ("MeanVsBG_pip","MeanVsBG_pip",250,250,650,560);
      
      c4->Divide(1,1);
      c4->cd(1);
      TH2F* frame3 = new TH2F("frame3","frame3",1000,0.9,2.15,1000,1.001,1.39);
      frame3->Draw();

      MeanVsBG_pip_c = (TGraphErrors*)MeanVsBG_pip->Clone("MeanVsBG_pip_c");

      MeanVsBG_pip_c->SetMarkerStyle(20);	
      MeanVsBG_pip_c->SetMarkerColor(32);
      MeanVsBG_pip_c->SetMarkerSize(0.9);
      //MeanVsBG_pip_c->Draw("same p");

      
      TF1* meanFitBG = new TF1(Form("Bethe_Bloch_pip"), BetheBlochVsBG, -0.5, 3, 5);
      meanFitBG -> SetParNames("X0", "X1", "miu", "s");
      meanFitBG -> SetLineColor(8);
      meanFitBG -> SetLineWidth(4);
      /*
      meanFitBG -> SetParameters(gBethe_Bloch_pi->GetParameter(0), gBethe_Bloch_pi->GetParameter(1),
				 gBethe_Bloch_pi->GetParameter(2), gBethe_Bloch_pi->GetParameter(3),
				 gBethe_Bloch_pi->GetParameter(4));
      */
      /*
      meanFitBG -> SetParameters(par_prot[0],par_prot[1],par_prot[2],par_prot[3],mass_prot);
      meanFitBG -> FixParameter(4,  mass_pi);
      */
      
      meanFitBG -> SetParameters(par_pion[0],par_pion[1],par_pion[2],par_pion[3],mass_pi);
      meanFitBG -> FixParameter(4,  mass_pi);
      
      MeanVsBG_pip_c -> Fit(meanFitBG, "", "", 1, 2.13);
      MeanVsBG_pip_c->Draw("same p");
      meanFitBG -> Draw("same");      
...
void DyVsY2(Int_t nSlicesInit, TH2F* h2, string particle, float &par1, float &par2, float &par3, float &par4)
{
  ...
  // pid directory
      ...
      if(particle == "proton")mass = gMass_prot;
      if(particle == "pion")  mass = gMass_pi;
...
//      nSlicesInit = sizeof(pstep_array) / 4;
      ...
      for (Int_t i = 0; i < nSlicesInit; i++) {
	    ...
	    //func = GetGaussFitPol_pik(slice[i], 0.5 * (pL + pU), params);
              func = GetGaussFitPol_pikel(slice[i], 0.5 * (pL + pU), params);
          }
          ...
          bgPart[i] = p - TMath::Log10(mass);        // because p is realy log10(p) then we define here log10(p/mass)
          bgPartErr[i] = bgPart[i] * 0.000001;
      ...
      TGraphErrors *MeanVsBG = new TGraphErrors(nSlices, bgPart, meanPart, bgPartErr, meanPartErr);
      TGraphErrors *SigmaVsBG = new TGraphErrors(nSlices, bgPart, sigmaPart, bgPartErr, sigmaPartErr);

      Mean->SetName(Form("Mean"));
      MeanVsBG->SetName(Form("MeanVsLogBG"));
      SigmaVsBG->SetName("SigmaVsLogBG");

      Mean->SetMarkerStyle(20);
      Mean->SetMarkerSize(1.3);
      Mean->SetMarkerColor(2);
      Mean->SetLineColor(2);
      Mean->SetLineWidth(3);

      MeanVsBG->SetMarkerStyle(20);
      MeanVsBG->SetMarkerSize(0.5);
      MeanVsBG->SetMarkerColor(4);
      MeanVsBG->SetLineColor(4);
      MeanVsBG->SetLineWidth(3);

      SigmaVsBG->SetMarkerStyle(20);
      SigmaVsBG->SetMarkerSize(0.5);
      SigmaVsBG->SetMarkerColor(4);
      SigmaVsBG->SetLineColor(6);
      SigmaVsBG->SetLineWidth(3);

 
      // PROTON
    TF1 *meanFit_pion = 0;

    TString specie = particle;

    TF1* meanFit = new TF1(Form("Bethe_Bloch_%s",specie.Data()), BetheBloch, -0.5, 3, 5);
    meanFit -> SetParNames("X0", "X1", "miu", "s");
    meanFit -> SetLineColor(3);
    meanFit -> SetLineWidth(4);
    meanFit -> SetParameters(gBethe_Bloch_pi->GetParameter(0), gBethe_Bloch_pi->GetParameter(1),
				gBethe_Bloch_pi->GetParameter(2), gBethe_Bloch_pi->GetParameter(3),
				gBethe_Bloch_pi->GetParameter(4));
    if(particle == "pion")   meanFit -> FixParameter(4,  gBethe_Bloch_pi->GetParameter(4));
    if(particle == "proton") meanFit -> FixParameter(4,  gBethe_Bloch_p->GetParameter(4));

    Mean -> Fit(meanFit, "w", "", pmin - 0.1, pmax + 0.1);
    
    
    Mean->GetListOfFunctions()->Add(meanFit);
    
    par1 = meanFit -> GetParameter(0);
    par2 = meanFit -> GetParameter(1);
    par3 = meanFit -> GetParameter(2);
    par4 = meanFit -> GetParameter(3);

    h2->GetListOfFunctions()->Add(Mean);
    h2->GetListOfFunctions()->Add(meanFit);

    h2->GetListOfFunctions()->Add(MeanVsBG);
    h2->GetListOfFunctions()->Add(SigmaVsBG);
...
func->SetParLimits(2,sig-0.015,sig+0.035);
...
//______________________________________________________________
TF1* GetGaussFitPol_pik(TH1D* h, Float_t p, const Double_t* params)
{
// h - histograms to fit
// params - pointer to list of parameters
// n - number of Gaussians to consider
...
cout<<"gBethe_Bloch_p->Eval(p) : "<<gBethe_Bloch_pi->Eval(p)<<" p="<<p<<endl;
 ...
 //func->FixParameter(4,gBethe_Bloch_k->Eval(p));
 func->SetParLimits(4,gBethe_Bloch_k->Eval(p)-0.1,gBethe_Bloch_k->Eval(p));
 func->FixParameter(5,0.065);

//func->SetParNames("const","mean","sigma","constB","meanB","sigmaB");
 func->SetParNames("const_pi","mean_pi","sigma_pi","const_k","mean_k","sigma_k");
...
//______________________________________________________________
TF1* GetGaussFitPol_pikel(TH1D* h, Float_t p, const Double_t* params)
{
// h - histograms to fit
// params - pointer to list of parameters
// n - number of Gaussians to consider
...
cout<<"gBethe_Bloch_p->Eval(p) : "<<gBethe_Bloch_pi->Eval(p)<<" p="<<p<<endl;
 ...
 //func->FixParameter(4,gBethe_Bloch_k->Eval(p));
 func->SetParLimits(7,gBethe_Bloch_el->Eval(p)-0.1,gBethe_Bloch_el->Eval(p)+0.1);
 func->FixParameter(8,0.065);

//func->SetParNames("const","mean","sigma","constB","meanB","sigmaB");
 func->SetParNames("const_pi","mean_pi","sigma_pi","const_k","mean_k","sigma_k","const_mu","mean_mu","sigma_mu");
...
Double_t BetheBlochVsBG(Double_t *x, Double_t *par) 
{
  ...
  // x + 4 paramtery Bethego-Blocha:
  Double_t mass = par[4];
  Double_t log10p = x[0] + TMath::Log10(mass);
  Double_t p = TMath::Power(10,log10p);
