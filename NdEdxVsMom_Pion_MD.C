...
void FitSlices(Int_t runid=1234, Int_t apver=23, Int_t nSlices=15, Int_t saveCalibParams=0)
{
  ...
  TList* list = hr_pi->GetListOfFunctions();
  ...
  gBethe_Bloch_p->SetLineColor(8);
  gBethe_Bloch_p->SetLineStyle(2);

  gBethe_Bloch_p->Draw("same");

  gBethe_Bloch_pi->SetLineColor(4);
  gBethe_Bloch_p->SetLineStyle(2);
...
void DyVsY2(Int_t nSlicesInit, TH2F* h2)
{
  ...
  for ( Int_t i = 0 ; i < nSlicesInit ; i++ ) {
    ...
    // CZY TUTAJ ZMIENIC ZEBY PIONY DOBRE SZLY?
  ...
  meanFit_prot->SetParameters(gBethe_Bloch_p->GetParameter(0),gBethe_Bloch_p->GetParameter(1), gBethe_Bloch_p->GetParameter(2), gBethe_Bloch_p->GetParameter(3), gBethe_Bloch_p->GetParameter(4));
  ...
  meanFit_pion->SetParameters(gBethe_Bloch_pi->GetParameter(0), gBethe_Bloch_pi->GetParameter(1), gBethe_Bloch_pi->GetParameter(2), gBethe_Bloch_pi->GetParameter(3), gBethe_Bloch_pi->GetParameter(4));
...
TF1* GetGaussFitPol(TH1D* h, Float_t p, const Double_t* params)
{
  ...
  cout<<"gBethe_Bloch_pi->Eval(p) : "<<gBethe_Bloch_pi->Eval(p)<<" p="<<p<<endl;
