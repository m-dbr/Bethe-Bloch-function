...
TF1* GetGaussFitPol(TH1D* h,const Double_t* params);
TF1* GetQuintupleGaussFit(TH1D* h,Float_t p,const Double_t* params);
void DrawSlice(TH1D* h,TCanvas* canv);
TF1* GetQuadrupleGaussFit(TH1D* h,Float_t p,const Double_t* params);
...
void FitSlices(Int_t runid=1234, Int_t nSlices=15, Int_t saveCalibParams=0)
{
  ...
  if(!gdEdx_pion){
    ...
    gdEdx_pion = new TF1("gdEdx_pion",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - %s)",beta2.Data(),
					   beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
    gdEdx_pion->SetParNames("A","B");
    // based on InvMass selection
    //A = 0.0566324;
    //B = 1.61401e-07;
    // 652-812
    //A = 0.0527338;
    ...
    A = 0.0599219;
    B = 5.62172e-07;
    gdEdx_pion->SetParameters(A,B); 
    
    ///// Proton dEdx
    beta2 = "(x*x/(x*x+0.880354496))";
    gdEdx_prot = new TF1("gdEdx_prot",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - %s)",beta2.Data(),
						beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
    ...
    gdEdx_kaon = new TF1("gdEdx_kaon",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - %s)",beta2.Data(),
						beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
    gdEdx_kaon->SetParNames("A","B");
    //A = 0.0814975;
    //B = 0.000114875;
    gdEdx_kaon->SetParameters(gdEdx_prot->GetParameter(0),gdEdx_prot->GetParameter(1)); 
    gdEdx_kaon->SetParameters(A,B); 

    ///// muon dEdx, mass = 0.1056583745 (mass^2 = 0.011163692102)
    beta2 = "(x*x/(x*x+0.0111636921))";
    gdEdx_muon = new TF1("gdEdx_muon",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - %s)",beta2.Data(),
						beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
    ...
    gdEdx_elec = new TF1("gdEdx_elec",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - %s)",beta2.Data(),
						beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
    ...
    gdEdx_deut = new TF1("gdEdx_deut",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - %s)",beta2.Data(),
						beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
    gdEdx_deut->SetParNames("A","B");
    gdEdx_deut->SetParameters(gdEdx_prot->GetParameter(0),gdEdx_prot->GetParameter(1));
  }


  TString dir = "SinglePartCutModule";  

  /*
  //TString fineName=Form("d0ana_run%d_p42_ap9.root",runid);
  TString fineName=Form("d0ana_run%d_p42_ap1.root",runid);
  //TFile* InputFile = TFile::Open(Form("../root_files/kfver3/%s",fineName.Data()));
  TFile* InputFile = TFile::Open(Form("../root_files/%s",fineName.Data()));
  //TFile* InputFile = TFile::Open(Form("./run_%d.root",runid));
  */
  ...
  //TH2F* hr = (TH2F*)InputFile->Get(Form("%s/hdEdxPos_mtpc",dir.Data()));
  //TH2F* hr = (TH2F*)InputFile->Get(Form("%s/hdEdxPos_all",dir.Data()));
  //TH2F* hr = (TH2F*)InputFile->Get(Form("%s/hdEdxPos_all",dir.Data()));
  TH2F* hr = (TH2F*)InputFile->Get(Form("%s/hdEdxNeg_all",dir.Data()));
  //TH2F* hr2 = (TH2F*)InputFile2->Get(Form("%s/hdEdxPos_all",dir.Data()));
  ...
  TGraphErrors* meank     = (TGraphErrors*)list->FindObject(Form("MeanK"));
  ...
  hrplot->GetListOfFunctions()->Add(meank);
...
void DyVsY(Int_t nSlices, TH2F* h2)
{
  ...
  */
  ...
  for ( Int_t i = 0 ; i < nSlices ; i++ ) {
    ...
    TF1* func = GetQuadrupleGaussFit(slice[i],0.5*(pL+pU),params);
    
    Bool_t drawSlices=kTRUE;
    if(drawSlices){
      cnv[i] = new TCanvas (Form("slice%d",i),Form("slice%d",i),100+5*i,5*i,400,560);
  ...
  TGraphErrors* MeanK = new TGraphErrors(nSlices,momPart,meanPartK,momPartErr,meanPartErr);        
  ...
  MeanK->SetMarkerStyle(20);
  MeanK->SetMarkerSize(0.05);
  MeanK->SetMarkerColor(3);
  MeanK->SetLineColor(2);
  MeanK->SetLineWidth(3);
  ...
  */
  ...
  //Mean->Fit(meanFit_pion,"w","",xmin-0.1,xmax+0.1);
  Mean->Fit(meanFit_pion,"w","",pmin-0.1,pmax+0.1);

  beta2 = "(x*x/(x*x+0.24371698))";
  TF1* meanFit_kaon = new TF1("meanFit_kaon",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - %s)",beta2.Data(),
						  beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
  meanFit_kaon->SetParNames("A","B");
  meanFit_kaon->SetLineColor(6);
  meanFit_kaon->SetLineWidth(4);
  meanFit_kaon->SetParameters(meanFit_pion->GetParameter(0),meanFit_pion->GetParameter(1)); 

  beta2 = "(x*x/(x*x+0.24371698))";
  TF1* meanFit_kaonf = new TF1("meanFit_kaonf",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - %s)",beta2.Data(),
						  beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
  meanFit_kaonf->SetParNames("A","B");
  meanFit_kaonf->SetLineColor(6);
  meanFit_kaonf->SetLineWidth(4);
  meanFit_kaonf->SetParameters(meanFit_pion->GetParameter(0),meanFit_pion->GetParameter(1)); 
  MeanK->Fit(meanFit_kaonf,"w","",pmin-0.1,pmax+0.1);
  ...
  TF1* meanFit_pion2 = new TF1("meanFit_pion2",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - %s)",beta2.Data(),
						  beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
  meanFit_pion2->SetParNames("A","B");
  meanFit_pion2->SetLineColor(kRed-2);
  meanFit_pion2->SetLineWidth(4);
  meanFit_pion2->SetParameters(0.0501404,7.01604e-09);
  
  // add graphs and fits to m2 histogram
  
  Mean->GetListOfFunctions()->Add(meanFit_pion);
  //Mean->GetListOfFunctions()->Add(meanFit_kaon);
  Mean->GetListOfFunctions()->Add(meanFit_prot);
  //Mean->GetListOfFunctions()->Add(meanFit_muon);
  //Mean->GetListOfFunctions()->Add(meanFit_elec);
  MeanK->GetListOfFunctions()->Add(meanFit_kaonf);
  //Mean->GetListOfFunctions()->Add(meanFit_deut);
  //Sigma->GetListOfFunctions()->Add(sigmaFit);
  
  h2->GetListOfFunctions()->Add(Mean);
  h2->GetListOfFunctions()->Add(MeanK);
...
TF1* GetQuadrupleGaussFit(TH1D* h,Float_t p,const Double_t* params)
{
  ...
  name.Append("_Fit");
  cout<<"name: "<<name.Data()<<endl;
  TF1* func = new TF1(name.Data(),Form("%s + %s + %s + %s",g1.Data(),g2.Data(),g3.Data(),g4.Data()),0.7,1.8);  
  ...
  func->SetParLimits(0,0,9000000);
  //func->SetParLimits(1,gdEdx_pion->Eval(p)-dd,gdEdx_pion->Eval(p)+2*dd);
  func->SetParLimits(1,gdEdx_pion->Eval(p)-ddd,gdEdx_pion->Eval(p)+ddd);
  Double_t beta = TMath::Sqrt(p*p/(p*p+0.01947983));
  //Float_t sigPi = 42.3956 - 42.344*beta; //1350-1450
  Float_t sigPi = 4.59047e+01 - 4.58552e+01*beta; //652-812
  if(beta<0.9989)sigPi = 0.1;

  func->SetParLimits(2,sigPi-0.001,sigPi+0.001);

  func->SetParLimits(3,0,900000);
  //func->FixParameter(3,0);
  func->SetParLimits(4,gdEdx_kaon->Eval(p)+dd-3*dd,gdEdx_kaon->Eval(p)+dd+10*dd);
  beta = TMath::Sqrt(p*p/(p*p+0.24371698));
  //Float_t sigKaon=42.3956 - 42.344*beta;
  //if(beta<0.9989)sigKaon = 0.1;
  sigKaon=0.055;
  func->SetParLimits(5,sigKaon-0.01,sigKaon+0.01);

  func->SetParLimits(6,0,9000000);
  //func->SetParLimits(7,gdEdx_prot->Eval(p)-10*dd,gdEdx_prot->Eval(p)+dd);
  func->SetParLimits(7,gdEdx_prot->Eval(p)-ddd,gdEdx_prot->Eval(p)+ddd);
  beta = TMath::Sqrt(p*p/(p*p+0.880354496));
  //Float_t sigProt = 50.3376 - 50.2852*beta;
  //Float_t sigProt=42.3956 - 42.344*beta;
  Float_t sigProt=0.055;
  func->SetParLimits(8,sigProt-0.001,sigProt+0.001);

  func->SetParLimits(9,0,70000);
  func->SetParLimits(10,gdEdx_pion->Eval(p)+4*sigPi-3*dd,gdEdx_pion->Eval(p)+4*sigPi+10*dd);
  func->SetParLimits(11,0.053-0.01,0.053+0.01);
  ...
  func->SetParLimits(1,func->GetParameter(1)+0.01-ddd,func->GetParameter(1)+0.01+ddd);
  func->SetParLimits(2,func->GetParameter(2)-0.001,func->GetParameter(2)+0.001);

  func->SetParLimits(4,func->GetParameter(4)+0.025-dd,func->GetParameter(4)+0.025+3*dd);
  func->SetParLimits(5,func->GetParameter(5)+0.01-0.001,func->GetParameter(5)+0.01+0.003);

  func->SetParLimits(7,func->GetParameter(7)-ddd,func->GetParameter(7)+ddd);
  func->SetParLimits(8,func->GetParameter(8)-0.001,func->GetParameter(8)+0.001);

  func->SetParLimits(10,func->GetParameter(10)-3*dd,func->GetParameter(10)+3*dd);
  func->SetParLimits(11,func->GetParameter(11)-0.01,func->GetParameter(11)+0.01);
...
TF1* GetGaussFitPol(TH1D* h,const Double_t* params)
{
  ...
  func->SetParLimits(0,0.5,90000);
  func->SetParLimits(1,-10.,15);
  func->SetParLimits(2,0.5,2.5);

  func->SetParLimits(3,0,200);
  func->SetParLimits(4,-1,1);
  func->SetParLimits(5,-0.001,0);
  ...
  Float_t sigmaErr = func->GetParError(2);
  if(sigmaErr > 0.5*sigma){
    cout<<"refitting histogram: sigma="<<sigma<<"  sigmaErr="<<sigmaErr<<endl;
    h->Fit(func,"w0","",rmin,rmax); 
  }

  //h->Fit(func,"","0",rmin,rmax);
  //h->Fit(func,"","",rmin,rmax);
  h->GetListOfFunctions()->Add(func);
