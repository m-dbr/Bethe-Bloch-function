...
void FitSlices(Int_t runid=1234, Int_t nSlices=15, Int_t saveCalibParams=0)
{
  ...
  if(!gdEdx_pion){
    ...
    Float_t pmin = 0.1;
    Float_t pmax = 50.0;
    // from InvMass selection 1359-1450
    //Double_t A = 0.0566324;
    //Double_t B = 1.61401e-07;
    // from InvMass selection 652-812
    Double_t A = 0.0527338;
    Double_t B = 2.96921e-08;
    //Double_t A = 0.0592363;
    //Double_t B = 1.65038e-07;

    TString beta2 = "(x*x/(x*x+0.01947983))";
    /*
    TString R = "1 + 2*(0.511/139.)/TMath::Sqrt(1-(x*x/(x*x+0.01947983)))";
    gdEdx_pion = new TF1("gdEdx_pion",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s)*%s) - 2*%s)",beta2.Data(),
					   beta2.Data(),beta2.Data(),beta2.Data(),R.Data()),pmin,pmax);
    */   
    ...
    gdEdx_prot->SetParameters(gdEdx_pion->GetParameter(0),gdEdx_pion->GetParameter(1)); 
  ...
  TString dir = "K0AnaModule";  

  //TString fineName=Form("d0ana_run%d_p42_ap9.root",runid);
  TString fineName=Form("d0ana_run1234_p5_ap23.root");
  ...
  TH2F* hr = (TH2F*)InputFile->Get(Form("%s/hPiPlus",dir.Data()));
  TH2F* hr1 = (TH2F*)InputFile->Get(Form("%s/hPiMinus",dir.Data()));

  dir = "LambAnaModule";  
  TH2F* hr2 = (TH2F*)InputFile->Get(Form("%s/hPiMinus",dir.Data()));
  TH2F* hr3 = (TH2F*)InputFile->Get(Form("%s/hPiPlus",dir.Data()));
  ...
  hrplot->GetListOfFunctions()->Add(meanPsigma);
...
void DyVsY(Int_t nSlices, TH2F* h2)
{
  ...
  */
  ...
  TString beta2 = "(x*x/(x*x+0.24371698))";
  TF1* meanFit_kaon = new TF1("meanFit_pion",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - %s)",beta2.Data(),
						  beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
  meanFit_kaon->SetParNames("A","B");
  meanFit_kaon->SetLineColor(3);
  meanFit_kaon->SetLineWidth(4);
  meanFit_kaon->SetParameters(0.07,2.8e-6); 
  Mean->Fit(meanFit_kaon,"w","",pmin-0.1,pmax+0.1);
...
void DyVsY2(Int_t nSlicesInit, TH2F* h2)
{
  ...
  meanFit_prot->SetLineColor(4);
  meanFit_prot->SetLineWidth(4);
  meanFit_prot->SetParameters(meanFit_pion->GetParameter(0),meanFit_pion->GetParameter(1));

  beta2 = "(x*x/(x*x+0.880354496))";
  TF1* meanFit_protP = new TF1("meanFit_protP",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - 2*%s)",beta2.Data(),
						    beta2.Data(),beta2.Data(),beta2.Data()),pmin,pmax);
  meanFit_protP->SetParNames("A","B");
  meanFit_protP->SetLineColor(3);
  meanFit_protP->SetLineWidth(4);
  Double_t AP = 0.077818;
  Double_t BP = 2.37957e-05; 
  meanFit_protP->SetParameters(AP,BP);
  
  beta2 = "(x*x/(x*x+0.24371698))";
  TF1* meanFit_kaon = new TF1("meanFit_kaon",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - 2*%s)",beta2.Data(),
						  beta2.Data(),beta2.Data(),beta2.Data()),pmin,3*pmax);
  meanFit_kaon->SetParNames("A","B");
  ...
  beta2 = "(x*x/(x*x+0.24371698))";
  TF1* meanFit_kaonP = new TF1("meanFit_kaonP",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - 2*%s)",beta2.Data(),
						    beta2.Data(),beta2.Data(),beta2.Data()),pmin,3*pmax);
  meanFit_kaonP->SetParNames("A","B");
  meanFit_kaonP->SetLineColor(7);
  meanFit_kaonP->SetLineWidth(4);
  //Double_t AP = 0.0776135;
  //Double_t BP = 7.11011e-05;
  //Double_t AP = 0.0779697;
  //Double_t BP = 6.61051e-05; 
  meanFit_kaonP->SetParameters(AP,BP);

  beta2 = "(x*x/(x*x+0.24371698))";
  TF1* meanFit_kaonPA = new TF1("meanFit_kaonPA",Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - 2*%s)",beta2.Data(),
						  beta2.Data(),beta2.Data(),beta2.Data()),pmin,3*pmax);
  meanFit_kaonPA->SetParNames("A","B");
  meanFit_kaonPA->SetLineColor(kOrange);
  meanFit_kaonPA->SetLineWidth(4);
  Double_t A = 0.5*(AP + meanFit_pion->GetParameter(0));
  Double_t B = 0.5*(BP + meanFit_pion->GetParameter(1));
  meanFit_kaonPA->SetParameters(A,B);
  ...
  Mean->GetListOfFunctions()->Add(meanFit_pion);
  Mean->GetListOfFunctions()->Add(meanFit_prot);
  Mean->GetListOfFunctions()->Add(meanFit_protP);
  //Mean->GetListOfFunctions()->Add(meanFit_elec);
  Mean->GetListOfFunctions()->Add(meanFit_kaon);
  Mean->GetListOfFunctions()->Add(meanFit_kaonP);
  //Mean->GetListOfFunctions()->Add(meanFit_kaonPA);
  
  h2->GetListOfFunctions()->Add(Mean);
  h2->GetListOfFunctions()->Add(SigmaVsMean);
  h2->GetListOfFunctions()->Add(SigmaVsBeta);
  //SigmaVsBeta->SetMarkerStyle(20);
  //SigmaVsBeta->SetMarkerSize(1.7);
  //SigmaVsBeta->SetMarkerColor(2);
  h2->GetListOfFunctions()->Add(SigmaVsBetaProt);
...
TF1* GetTripleGaussFit(TH1D* h,Float_t p,const Double_t* params)
{
  ...
  name.Append("_Fit");
  cout<<"name: "<<name.Data()<<endl;
  TF1* func = new TF1(name.Data(),Form("%s + %s + %s",g1.Data(),g2.Data(),g3.Data()),0.,1.8);  
  //TF1* func = new TF1(name.Data(),Form("%s + %s",g1.Data(),g2.Data()),-10.,10.,6);  
...
TF1* GetGaussFitPol(TH1D* h, Float_t p, const Double_t* params)
{
  ...
  func->SetParLimits(1,gdEdx_pion->Eval(p)-4*dd, gdEdx_pion->Eval(p)+20*dd);
  //func->SetParLimits(1,0.8, 1.5);
  func->SetParLimits(2,sig-0.015,sig+0.15);
  ...
  func->SetParLimits(1,func->GetParameter(1)-5*dd, func->GetParameter(1)+5*dd);
  func->SetParLimits(2,func->GetParameter(2)-0.05,func->GetParameter(2)+0.35);
