...
void FitSlices(Int_t runid=1234, Int_t apver=5, Int_t nSlices=15, Int_t saveCalibParams=0)
{

  gStyle->SetOptStat(1101); // displays the name of histogram, mean value and RMS
  gStyle->SetOptFit(1101); // The type of information about fit parameters printed in the histogram statistics box
  ...
  if(!gdEdx_pion){
    ...
    // gdEdx_prot = new TF1("gdEdx_prot", Form("[0]/%s * (TMath::Log(%s/([1]*(1-%s))) - 2*%s)", beta2.Data(), beta2.Data(), beta2.Data(), beta2.Data()), pmin, pmax);
    // gdEdx_prot->SetParNames("A", "B");
    // gdEdx_prot->SetParameters(A, B); 

    // W nowej parameteryzacji mamy 4 parametry: s, miu, X0 i X1
    // gdEdx_prot_NEW = BetheBloch(Double_t *x, Double_t *par);
    // TF1 *Bethe_Bloch_func = new TF1("Bethe Bloch", BetheBloch, -3, 3, 3); // <- ostatnie trzy cyfry to liczba parametrow
    // // set the parameters to the mean and RMS of the histogram
    // Bethe_Bloch_func->SetParameters(500,hpx->GetMean(),hpx->GetRMS());
    // func->SetParNames ("Constant","Mean_value","Sigma")

    TF1* Bethe_Bloch_func = new TF1("Bethe Bloch", BetheBloch_p, -0.5, 3, 4); // <- ostatnie trzy cyfry to liczba parametrow
    // set the parameters to the mean and RMS of the histogram
    Bethe_Bloch_func->SetParameters(par);
    Bethe_Bloch_func->SetParNames("X0","X1","miu","s");
    ...
    TF1* Bethe_Bloch_func_pi = new TF1("Bethe Bloch pi", BetheBloch_pi, -0.5, 3, 4); // <- ostatnie trzy cyfry to liczba parametrow
    // set the parameters to the mean and RMS of the histogram
    Bethe_Bloch_func_pi->SetParameters(par);
    Bethe_Bloch_func_pi->SetParNames("X0","X1","miu","s");
  ...
  TString fineName=Form("d0ana_run1234_p5_ap%d.root",apver);
...
// powinno opisywac tak jak funkcja dla meanFit_pion - meanFit_pion zle opisuje 
